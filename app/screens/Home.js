...
import { StyleSheet, Text, View, BackHandler, Dimensions, TouchableOpacity, TouchableHighlight, ImageBackground, Image, ScrollView, ActivityIndicator} from 'react-native';
...
const DeviceList = ({ devices , connectedId, showConnectedIcon=false, onDevicePress }) =>
  <ScrollView>
      ...
      {devices.map((device, i) => {
        return (
          <TouchableHighlight
            ...
            key={`${device.id}_${i}`}
            style={st.listItem} onPress={() => onDevicePress(device)}>
    ...
    render() {
        return (
            <View style={st.container}>
                ...
                <ScrollView contentContainerStyle={st.body} style={{flex: 1,marginTop: -1 * (costomHeight / 2)}}>
                    <View style={st.itemRows}>
                        <TouchableOpacity onPress={() => {}} elevation={4} style={st.item}>
                            <Image style={{width: 50, height: 50, marginBottom: 5}} source={require("./../../images/helpicon.png")} />
                    ...
                    <View style={st.itemRows}>
                        <TouchableOpacity onPress={() => this.props.navigation.navigate('DrugPlans')} elevation={4} style={st.item}>
                            <Image style={{width: 50, height: 50, marginBottom: 5}} source={require("./../../images/helpicon.png")} />                            
                            <Text style={st.itemText} >مشاهده برنامه دارویی</Text>
                        </TouchableOpacity>
                        <TouchableOpacity onPress={() => this.props.navigation.navigate('EmergencyCall') } elevation={4} style={st.item}>
                            <Image style={{width: 50, height: 50, marginBottom: 5}} source={require("./../../images/helpicon.png")} />                            
                    ...
                    <View style={st.itemRows}>
                        ...
                        <TouchableOpacity onPress={() => this.props.navigation.navigate('GeneralSettings')} elevation={4} style={st.item}>
                            <Image style={{width: 50, height: 50, marginBottom: 5}} source={require("./../../images/helpicon.png")} />                            
                            <Text style={st.itemText} >تنظیمات</Text>
                        </TouchableOpacity>
                    </View>
                </ScrollView>
                <TouchableOpacity onPress={() => BackHandler.exitApp()} style={st.exit}>
                    <Text style={[st.itemText, {color: '#fff', paddingVertical: 7, fontSize: 18}]} >خروج</Text>
                </TouchableOpacity>
                <Overlay visible={this.state.connected ? false: true} 
                    animationType="zoomIn"
                    containerStyle={{backgroundColor: 'rgba(84, 160, 255, 0.40)'}}
                    childrenWrapperStyle={{backgroundColor: '#eee'}}
                    animationDuration={300}>
                    {this.state.discovering ? 
                        
                        <View>
                            <Text style={[st.itemText, {paddingBottom: 10}]} >در حال جستجوی دستگاه ...</Text>
                            <ActivityIndicator size="large" color="#54a0ff" />
                        </View>
                    :
                        <DeviceList
                            connectedId={this.state.device && this.state.device.id}
                            devices={this.state.unpairedDevices.concat(this.state.devices)}
                            onDevicePress={(device) => this.onDevicePress(device)} />
                    }
                    {this.state.discovering ? 
                        <Button title="لغو جستجو" onPress={this.cancelDiscovery.bind(this)}  />
                    :
                        <Button title="تلاش دوباره" onPress={this.discoverUnpaired.bind(this)} />
                    }
