...
import { Platform, StyleSheet, Text, View, Switch, TextInput, TouchableHighlight } from 'react-native';
        ...
        DB.transaction((tx) => {
            for (var key in this.state.setting) {
                if (this.state.setting.hasOwnProperty(key)){
                    // console.log(key);
                    let keyy = key
                    tx.executeSql("SELECT * FROM settings where `key` = ?", [keyy], (tx, res) => {
    ...
    _savePhoneNumbers = () => {
        ...
        for (var key in this.state.setting) {
            if (this.state.setting.hasOwnProperty(key)){
                if (key == 'setdate') {
                    var d = new Date();
                    if (this.state.setting['setdate'] == '0.0') {
                        text += "#date-" + d.getFullYear() + (d.getMonth() + 1) + d.getDate();
                        text += "#time-" + d.getHours() + d.getMinutes;                                
                    }else{
                        text += "#date-" + d.getFullYear() + (d.getMonth() + 1) + d.getDate();
                        text += "#time-" + this.state.hour + this.state.min;
                    }
                }else{
                    text += "#" + key + "-" + (this.state.setting[key] == '1.0' ? 1 : 0);                                
                }
            }
        }
        // console.log(text);
                ...
                if (this.state.setting.hasOwnProperty(key)){
                    ...
                    tx.executeSql("SELECT * FROM settings where `key` = ?", [keyy], (tx, res) => {
                        ...
                        if (res.rows.length > 0) {
                            tx.executeSql("UPDATE settings SET value= ? where `id`= ?", [this.state.setting[keyy], res.rows.item(0).id])
                            var num = {};
                            num[keyy] = this.state.setting[keyy];
                            this.setState(num);
                        } else {
                            tx.executeSql("insert into settings (`key`, value) values (?, ?)", [keyy, this.state.setting[keyy]]);
                        }
                    })
                }
            }
                ...
                <View style={st.body}>
                    <View style={{flexDirection: 'row', alignItems: 'center'}} >
                        <Switch
                            ...
                            value={this.state.setting.ring == "1.0" ? true : false}
                        />
                        <Text style={st.settingText} > -  زنگ خوردن خود دستگاه</Text>
                    </View>
                    <View style={{flexDirection: 'row', alignItems: 'center'}} >
                        <Switch
                            onTintColor="#43a9ff"
                            ...
                            value={this.state.setting.helpLight > 0 ? true : false}
                        />
                        <Text style={st.settingText} > -  چراغ های مصرف دارو</Text>
                    </View>
                    <View style={{flexDirection: 'row', alignItems: 'center'}} >
                        <Switch
                            onTintColor="#43a9ff"
                            ...
                            value={this.state.setting.waterOnOff > 0 ? true : false}
                        />
                    ...
                    <View style={{flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginBottom: 10}} >
                        ...
                        <Switch
                            ...
                            value={this.state.setting.setdate > 0 ? true : false}
                        />
